...
// ScenarioTestTraceNoBackend10kSPS runs a performance test at 10k sps with specified processor
func ScenarioTestTraceNoBackend10kSPS(
	t *testing.T,
	sender testbed.DataSender,
	receiver testbed.DataReceiver,
	resourceSpec testbed.ResourceSpec,
	configuration testbed.ProcConfiguration,
) {
	...
	configFile := createConfigFile(sender, receiver, resultDir, configuration.Processor)
	defer os.Remove(configFile)
